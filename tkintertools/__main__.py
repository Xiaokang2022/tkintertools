...
class BaseWidget:
    ...
    def __init__(
        ...
        * `tick`: 复选框控件的标记符号，默认为一个“对号”
        ...
        self.command_ex = {'normal': None, 'touch': None, 'click': None, 'disabled': None}  # type: dict[str, Callable | None]
    ...
    def state(self, mode=None):  # type: (Literal['normal', 'touch', 'click', 'disabled'] | None) -> str | None
        """
...
class Switch:
    ...
    def _animate(self, ms=SWITCH_ANIMATION_MS):  # type: (int) -> None
        ...
        if self.value:
            key = 1
            self.outside.configure(color_fill=COLOR_SWITCH_ON, color_outline=COLOR_SWITCH_ON)
        else:
            key = -1
            self.outside.configure(color_fill=COLOR_SWITCH_OFF, color_outline=COLOR_BUTTON_OUTLINE)
        self.outside.state()
        Animation(self.inside, ms, fps=90, control=(math.sin, 0, math.pi), translation=(key * (self.width - self.height) * self.inside.master.rx, 0)).run()
    ...
    def zoom(self, rate_x, rate_y, *, precision=None):  # type: (float, float, ..., float | None) -> tkinter.PhotoImage
        ...
        * `precision`: 精度到小数点后的位数（推荐 1.2），越大运算就越慢（默认值代表绝对精确）
        ...
        if precision is not None:
            ...
            rate_x = Fraction(str(rate_x)).limit_denominator(limit)  # type: Fraction
            rate_y = Fraction(str(rate_y)).limit_denominator(limit)  # type: Fraction
...
class Animation:
    ...
    def _parts(self, control, up, down):  # type: (Callable[[float], float], float, float) -> list[float]
        """部分比率"""
...
def SetProcessDpiAwareness(awareness=PROCESS_SYSTEM_DPI_AWARE):  # type: (Literal[0, 1, 2]) -> None
    """
