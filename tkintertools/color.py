...
def color(
    ...
    /,
    proportion: float = constants.PROPORTION,
    *,
    seqlength: int = constants.SEQLENGTH,
    num: typing.Literal[1, 2, 3, 4] = constants.NUM,
    controller: tuple[typing.Callable[[float], float],
                      float, float] | None = constants.CONTROLLER,
) -> str | list[str]: ...
...
def color(
    ...
    * `controller`: 比例值变化的控制器，为元组 (控制函数, 初始值, 终止值)，仅当参数 `seqlength` 大于 1 时生效，默认为等差变化
    ...
    proportion_lst = _tools.get_control_lst(controller, seqlength)
